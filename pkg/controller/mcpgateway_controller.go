...
const (
	BrokerConfigNamespace = "mcp-system"
	BrokerConfigName      = "mcp-broker-config"
)
...
func (r *MCPGatewayReconciler) regenerateAggregatedConfig(ctx context.Context) (reconcile.Result, error) {
	...
	if len(mcpGatewayList.Items) == 0 {
		log.Info("No MCPGateways found, deleting ConfigMap")
		configMap := &corev1.ConfigMap{}
		err := r.Get(ctx, types.NamespacedName{
			Name:      BrokerConfigName,
			Namespace: BrokerConfigNamespace,
		}, configMap)
		if err == nil {
...
func (r *MCPGatewayReconciler) writeAggregatedConfig(ctx context.Context, brokerConfig *config.BrokerConfig) error {
	...
	return writer.WriteAggregatedConfig(ctx, BrokerConfigNamespace, BrokerConfigName, brokerConfig)
}
