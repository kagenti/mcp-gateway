...
func (r *MCPServerReconciler) cleanupOrphanedHTTPRoutes(
	...
	if err := r.List(ctx, httpRouteList, client.MatchingFields{"status.hasAffectedCondition": "true"}); err != nil {
		return fmt.Errorf("failed to list affected HTTPRoutes: %w", err)
	}
	...
	for _, httpRoute := range httpRouteList.Items {
		...
		hasAffectedCondition := false
		updateNeeded := false
		for i, parentStatus := range httpRoute.Status.Parents {
			newConditions := []metav1.Condition{}
			for _, condition := range parentStatus.Conditions {
				if condition.Type == "Affected" && condition.Status == metav1.ConditionTrue {
					hasAffectedCondition = true
					updateNeeded = true
				} else {
		...
		if hasAffectedCondition {
			log.Info("Cleaning up Affected condition on orphaned HTTPRoute",
...
func (r *MCPServerReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	if err := mgr.GetFieldIndexer().IndexField(context.Background(), &gatewayv1.HTTPRoute{}, "status.hasAffectedCondition", func(rawObj client.Object) []string {
