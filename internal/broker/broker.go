...
func (m *mcpBrokerImpl) GetServerInfo(tool string) *config.MCPServer {
	for _, upstream := range m.mcpServers {
		rawTool := strings.TrimPrefix(tool, upstream.UpstreamMCP.ToolPrefix)
		t := upstream.GetManagedTool(rawTool)
		if t != nil {
			slog.Info("[EXT-PROC] Found matching server",
				"toolName", tool,
				"serverPrefix", upstream.UpstreamMCP.ToolPrefix,
				"serverName", upstream.UpstreamMCP.Name)
			return upstream.UpstreamMCP.MCPServer
		}
	}
