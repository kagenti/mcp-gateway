...
func TestRemoveHeader(t *testing.T) {
	testCases := []struct {
		...
		Headers         *corev3.HeaderMap
		KeyToRemove     string
		ExpectedHeaders []*corev3.HeaderValue
	}{
		{
			Name: "removes single matching header",
			Headers: &corev3.HeaderMap{
				Headers: []*corev3.HeaderValue{
					{Key: "x-test", RawValue: []byte("value1")},
					{Key: "x-keep", RawValue: []byte("value2")},
				},
			},
			KeyToRemove: "x-test",
			ExpectedHeaders: []*corev3.HeaderValue{
		...
		{
			Name: "removes multiple matching headers",
			Headers: &corev3.HeaderMap{
				Headers: []*corev3.HeaderValue{
					{Key: "x-duplicate", RawValue: []byte("value1")},
					{Key: "x-keep", RawValue: []byte("value2")},
					{Key: "x-duplicate", RawValue: []byte("value3")},
				},
			},
			KeyToRemove: "x-duplicate",
			ExpectedHeaders: []*corev3.HeaderValue{
				{Key: "x-keep", RawValue: []byte("value2")},
			},
		},
		{
			Name: "does nothing when key not found",
			Headers: &corev3.HeaderMap{
				Headers: []*corev3.HeaderValue{
					{Key: "x-keep1", RawValue: []byte("value1")},
					{Key: "x-keep2", RawValue: []byte("value2")},
				},
			},
			KeyToRemove: "x-nonexistent",
			ExpectedHeaders: []*corev3.HeaderValue{
				{Key: "x-keep1", RawValue: []byte("value1")},
				{Key: "x-keep2", RawValue: []byte("value2")},
			},
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			mcpReq.RemoveHeader(tc.KeyToRemove)

			if tc.ExpectedHeaders == nil {
				require.Nil(t, mcpReq.Headers)
			} else {
				require.Equal(t, len(tc.ExpectedHeaders), len(mcpReq.Headers.Headers))
				for i, expectedHeader := range tc.ExpectedHeaders {
					require.Equal(t, expectedHeader.Key, mcpReq.Headers.Headers[i].Key)
					require.Equal(t, expectedHeader.RawValue, mcpReq.Headers.Headers[i].RawValue)
				}
			}
		})
	}
